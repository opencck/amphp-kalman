# Time-varying beta (dynamic CAPM)

*Русская версия: [time-varying-beta.ru.md](time-varying-beta.ru.md).*

Class: `Domain\Model\Finance\TimeVaryingBeta`.
...
Structurally identical to pairs trading, but on returns.
`β̂_t ± √P_ββ` (`TimeVaryingBeta::betaBand($filter, k)`) is the beta corridor
for hedging a portfolio.

## Choosing q_β

From "beta changes by Δ over a horizon T": `q_β = Δ²/T`
(`TimeVaryingBeta::betaNoiseFromHorizon(0.1, 90 * 86400)`).
